import json, os, threading, time, logging, decimal, datetime as _dt
...
def _json_safe(obj):
    """Recursively coerce Decimal/numpy/datetime/bytes into JSON-friendly types."""
    try:
        import numpy as _np
    ...
    if isinstance(obj, bytes):
        try:
            ...
            return repr(obj)
    if _np_scalar and isinstance(obj, _np_scalar):
        try:
            return obj.item()
        except Exception:
            return str(obj)
    ...
    try:
        ...
        if status in ('closed', 'canceled', 'expired', 'rejected'):
            ...
            if filled_amt > 0 and status == 'closed':
                if po['side'] == 'buy':
                    state['position'] = {
                        'symbol':po['symbol'], 'amount':filled_amt, 'entry':avg,
                        'stop':avg - po['atr'] * float(state['stop_atr']),
                        'target':avg + po['atr'] * float(state['target_atr']),
        ...
        state['last_error'] = f"Order reconciliation failed: {e}"
        save()
    return None
    ...
    for _ in range(8):
        try:
            ...
            if status == 'closed':
                ...
                record(('SHORT' if direction == 'SHORT' else side.upper()),
                       symbol, filled_amt, avg, reason, o)
                save()
                return o
...
def buy(symbol, price, atr, reason):
    try:
        ...
        save()
        logging.exception('Kraken account/balance check failed before BUY')
        return False
    amount = calculate_amount(symbol, price, atr, a['equity'])
    if not armed():
        amount = min(amount, float(state['paper_cash']) / price)
    ...
    state['position'] = {
        ...
        'stop': price - atr * float(state['stop_atr']),
        'target': price + atr * float(state['target_atr']),
...
def short(symbol, price, atr, reason):
    ...
    try:
        ...
        save()
        logging.exception('Account check failed before SHORT')
        return False
    ...
    if armed():
        try:
            ...
            save()
            logging.exception('Kraken SHORT failed')
            return False
        ...
        try:
            ...
            if direction == 'SHORT':
                o = kraken.margin_order(
                    symbol, 'buy', amount,
                    leverage=max(1, int(pos.get('leverage', state.get('short_leverage', 2)))),
                    close=True
                )
            else:
            ...
            state['pending_order'] = None
            state['last_error'] = None
            db.audit(now(), 'close', {'symbol': symbol, 'direction': direction,
                                      'pnl': pnl, 'reason': reason, 'order_id': o.get('id')})
            save()
            return True
        except Exception as e:
            state['last_error'] = f'Kraken close failed: {e}'
            save()
            logging.exception('Kraken close failed')
            return False
...
def scan():
    ...
    for symbol in ALLOWED_SYMBOLS:
        try:
            ...
            if action == 'BUY':
                score = 100 + max(0, score)
            elif action == 'SHORT':
                score = 100 + max(0, -score)
            strength = min(100, abs(score))
            rows.append({
                'symbol': symbol, 'action': action, 'reason': r['reason'],
                'price': float(d.close), 'ema': float(d.ema),
                'upper': float(d.upper), 'lower': float(d.lower),
                'atr': float(d.atr), 'score': score, 'strength': strength
            })
        except Exception as e:
            rows.append({'symbol': symbol, 'action': 'ERROR', 'reason': str(e),
                         'score': -1, 'strength': 0})
...
# ------------------------------------------------------------------
# Protective orders / position guard
# ------------------------------------------------------------------
def ensure_protective_orders():
    ...
    try:
        ...
        pos['tp_present'] = has_tp
        pos['sl_present'] = has_sl
        ...
        save()
        logging.exception('Protective order verification failed')
        return False
...
def reconcile_existing_positions():
    try:
        ...
        save()
        logging.exception('Kraken position reconciliation failed')
        return False
    ...
    for p in positions:
        if p.get('symbol') or p.get('asset'):
            candidates.append({
                ...
                'entry': state.get('position', {}).get('entry', 0)
                         if state.get('position', {}).get('symbol') == p['symbol'] else 0,
                'source': 'spot'
            })
    ...
    if candidates:
        ...
        if entry <= 0:
            try:
                entry = float(kraken.ticker(selected['symbol'])['last'])
            except Exception:
                entry = 0
        ...
        if atr <= 0:
            try:
                ...
                d = indicators(df, int(state['ema_period']), int(state['atr_period']),
                               float(state['keltner_multiplier'])).iloc[-2]
                atr = float(d.atr)
            except Exception:
                atr = entry * 0.01
        ...
        side = selected['side']
        if side == 'SHORT':
            stop = entry + atr * float(state['stop_atr'])
            target = entry - atr * float(state['target_atr'])
        else:
            stop = entry - atr * float(state['stop_atr'])
            target = entry + atr * float(state['target_atr'])
    ...
    try:
        ...
        if pos:
            ...
            if armed() and exchange_position_guard():
                state['position_locked'] = True
                state['position_lock_reason'] = state.get('trading_locked_reason') or \
                    'Existing Kraken exposure detected; no new trade will be placed.'
                state['last_scan_time'] = now()
                save()
                return
...
def api_recent_trades():
    try:
        ...
        logging.exception('Recent trades database read failed')
        return jsonify({'ok': False, 'source': 'trading.db', 'trades': [], 'error': str(e)}), 200


@app.get('/api/open_trades')
def api_open_trades():
    """Read-only Kraken reconciliation. Always returns valid JSON (200)."""
    try:
        if not os.getenv('KRAKEN_API_KEY') or not os.getenv('KRAKEN_API_SECRET'):
            return jsonify({'ok': False, 'source': 'Kraken', 'trades': [],
                            'error': 'Kraken API credentials are not configured'})

        # ---- Balance ----
        try:
            balance = kraken.balance() or {}
        except Exception as e:
            return jsonify({'ok': False, 'source': 'Kraken', 'trades': [],
                            'error': f'Kraken balance() failed: {e}'})
        total = balance.get('total') or {}
        free = balance.get('free') or {}

        # ---- Trade history (optional; failure must not kill the endpoint) ----
        history = []
        history_error = None
        try:
            ex = getattr(kraken, 'exchange', None)
            if ex is None or not hasattr(ex, 'fetch_my_trades'):
                history_error = 'kraken.exchange.fetch_my_trades is not available'
            else:
                history = ex.fetch_my_trades(None, limit=1000) or []
        except Exception as e:
            history = []
            history_error = str(e)

        # ---- Symbol universe (optional; failure must not kill the endpoint) ----
        known_symbols = []
        try:
            ex = getattr(kraken, 'exchange', None)
            if ex is not None and hasattr(ex, 'symbols'):
                raw = ex.symbols
                if isinstance(raw, dict):
                    known_symbols = [str(k) for k in raw.keys()]
                elif isinstance(raw, (list, tuple, set)):
                    known_symbols = [str(s) for s in raw]
            if not known_symbols and hasattr(kraken, 'symbols'):
                try:
                    known_symbols = [str(s) for s in (kraken.symbols('USDT', 500) or [])]
                except Exception:
                    known_symbols = []
        except Exception as e:
            logging.warning('Could not enumerate Kraken symbols: %s', e)
            known_symbols = []

        known_upper = {s.upper(): s for s in known_symbols}

        # ---- FIFO lots from executed trades ----
        lots = {}
        for t in sorted(history or [], key=lambda x: (x.get('timestamp') or 0, str(x.get('id') or ''))):
            try:
                ...
                try:
                    qty = abs(float(t.get('amount') or 0))
                except Exception:
                    qty = 0
                try:
                ...
                if side == 'buy':
                    bucket.append({
                        'symbol': symbol, 'amount': qty, 'entry': price,
                        'cost': float(t.get('cost') or (qty * price)),
                        'time': t.get('datetime') or t.get('time') or None,
                        'trade_id': t.get('id')
                    })
                else:
                    remaining = qty
        ...
        min_value = 1.0
        try:
            min_value = float(os.getenv('MIN_POSITION_VALUE_USDT', '1.0'))
        except Exception:
            pass

        for asset, raw_total in (total or {}).items():
            try:
                asset_u = str(asset).upper()
                if asset_u in {'USDT', 'USD'}:
                    continue
                qty = float(raw_total or 0)
                if qty <= 0:
                    continue

                symbol = known_upper.get(f'{asset_u}/USDT') or known_upper.get(f'{asset_u}/USD')
                if not symbol:
                ...
                current = 0.0
                try:
                    ticker = kraken.ticker(symbol) or {}
                    current = float(ticker.get('last') or ticker.get('close') or 0)
                except Exception:
                    current = 0.0

                if current > 0 and qty * current < min_value:
                    continue
                ...
                if remaining > 1e-12:
                    open_lots.append({
                        'symbol': symbol, 'amount': remaining, 'entry': 0.0,
                        'cost': 0.0, 'time': None, 'trade_id': None,
                        'unknown_entry': True
                    })

                for lot in open_lots:
                    amount = float(lot['amount'])
                    entry = float(lot.get('entry') or 0)
                    ...
                    result.append({
                        'symbol': symbol, 'asset': asset_u, 'side': 'LONG',
                        'amount': amount, 'entry': entry,
                        'current_price': current,
                        'open_value': open_value, 'current_value': current_value,
                        'pnl': pnl, 'pnl_pct': pnl_pct,
                        'opened': lot.get('time'), 'trade_id': lot.get('trade_id'),
                        'unknown_entry': bool(lot.get('unknown_entry', False))
                    })
            except Exception as e:
                logging.warning('open_trades row failed for asset %r: %s', asset, e)
                continue
        ...
        for row in result:
            try:
                ...
                for o in orders:
                    ...
                    if px is None:
                        try:
                            px = float((o.get('info') or {}).get('price') or 0)
                        except Exception:
                            px = 0
                    if 'take-profit' in typ or 'take profit' in desc:
                        row['tp'] = float(px or 0)
                        row['tp_order_id'] = o.get('id')
                    elif 'stop-loss' in typ or 'stop loss' in desc:
                        row['sl'] = float(px or 0)
                        row['sl_order_id'] = o.get('id')
            except Exception:
                continue

        return jsonify(_json_safe({
            'ok': True, 'source': 'Kraken',
            'trades': result,
            'history_error': history_error
        }))
    except Exception as e:
        logging.exception('Kraken open trade dashboard reconciliation failed')
        return jsonify({'ok': False, 'source': 'Kraken', 'trades': [], 'error': str(e)})


@app.get('/api/balance')
def api_balance():
    if not armed():
        return jsonify({'ok': False, 'mode': 'PAPER', 'usdt_balance': float(state['paper_cash'])})
    try:
        b = kraken.balance()
        free = b.get('free') or {}
        total = b.get('total') or {}
        return jsonify({'ok': True, 'mode': 'LIVE',
                        'USDT': {'free': float(free.get('USDT', 0) or 0),
                                 'total': float(total.get('USDT', 0) or 0)}})
    except Exception as e:
...
@app.get('/api/markets')
def markets():
    q = request.args.get('quote', 'GBP')
    return jsonify({'symbols': kraken.symbols(q, 100)})
    ...
    if enable:
        if not ENV_FILE.exists():
            return jsonify({'ok': False, 'armed': False,
                            'error': f'.env file not found at {ENV_FILE}'}), 400
        if not _env_true('LIVE_TRADING'):
            return jsonify({'ok': False, 'armed': False,
        ...
        try:
            ...
            return jsonify({'ok': False, 'armed': False,
                            'error': f'Kraken authentication/balance check failed: {msg}'}), 400
...
def start_goal():
    ...
    state['running'] = True
    save()
    return jsonify({'ok': True, 'goal_active': True, 'running': True,
                    'start_gbp': 10.0, 'target_gbp': 100.0})
...
def settings():
    ...
    allowed = set(DEFAULT) - {'position', 'trades', 'running', 'last_scan', 'last_signal',
                              'last_error', 'symbols', 'quotes', 'scanner_status', 'selected_pair'}
    with lock:
        for k, v in d.items():
            if k in allowed:
                state[k] = v
...
def close():
    try:
        return jsonify({'ok': sell('Manual close')})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def api_backtest():
    d = request.get_json(force=True)
    symbol = d.get('symbol', state['symbols'][0])
    timeframe = d.get('timeframe', state['timeframe'])
    df = kraken.ohlcv(symbol, timeframe, int(d.get('limit', 1000)))
    result = backtest(
        df,
        int(d.get('ema_period', state['ema_period'])),
        int(d.get('atr_period', state['atr_period'])),
        float(d.get('multiplier', state['keltner_multiplier'])),
        float(d.get('stop_atr', state['stop_atr'])),
        float(d.get('target_atr', state['target_atr'])),
        float(d.get('fee_pct', state['fee_pct'])),
        float(d.get('slippage_pct', state['slippage_pct'])),
        bool(d.get('trend_filter', state['trend_filter']))
    )
    db.audit(now(), 'backtest', {'symbol': symbol, 'timeframe': timeframe, 'result': result})
...
def api_optimize():
    ...
    best = None
    results = []
    emas = d.get('emas', [10, 15, 20, 30, 40])
    atrs = d.get('atrs', [7, 10, 14])
    mults = d.get('multipliers', [1.5, 2.0, 2.5, 3.0])
    for e in emas:
        for a in atrs:
            for m in mults:
                r = backtest(
                    df, e, a, m,
                    float(state['stop_atr']), float(state['target_atr']),
                    float(state['fee_pct']), float(state['slippage_pct']),
                    bool(state['trend_filter'])
                )
                item = {'ema': e, 'atr': a, 'multiplier': m,
                        **{k: r[k] for k in ('return_pct', 'trades', 'win_rate_pct', 'max_drawdown_pct')}}
                results.append(item)
                if r['trades'] >= 5 and (best is None or r['return_pct'] > best['return_pct']):
                    best = item
    return jsonify({'best': best,
                    'results': sorted(results, key=lambda x: x['return_pct'], reverse=True)[:30]})
